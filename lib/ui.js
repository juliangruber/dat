...
module.exports.progress = function (percent) {
  ...
  var percentMsg = ' ' + chalk.dim(Math.round(percent * 100) + '%')

  if (percent >= 1) return ends[0] + chalk.green(progressVal) + ends[1] + percentMsg
  else return ends[0] + chalk.blue(progressVal) + ends[1] + percentMsg
}
